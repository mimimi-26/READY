...
export default function App() {
  ...
  const menu = [
    ["home", "홈"], ["guide", "사용 가이드"], ["analyze", "경험 분석"], ["import", "파일 가져오기"], ["archive", "경험 보관함"], ["skills", "역량·스킬"], ["apply", "지원 관리"], ["resume", "기본 이력서"],
  ];
  ...
  return (
    <div style={{ fontFamily: font, background: C.bg, minHeight: "100vh", display: "flex", color: C.text }}>
      ...
      <main style={{ flex: 1, padding: "26px 32px", maxWidth: 1120, minWidth: 0 }}>
        ...
        {nav === "archive" && !detailId && <Archive experiences={experiences} setExperiences={setExperiences} onOpen={openDetail} onAnalyze={openAnalyze} onGoImport={() => go("import")} />}
        {nav === "archive" && detailId && <ExperienceDetail exp={experiences.find(e => e.id === detailId)} metrics={metrics} outputs={outputs} setOutputs={setOutputs} setExperiences={setExperiences} onBack={() => setDetailId(null)} onAnalyze={openAnalyze} onDeleted={() => setDetailId(null)} />}
        ...
        {nav === "skills" && <Skills skills={skills} setSkills={setSkills} certs={certs} setCerts={setCerts} experiences={experiences} onOpenExp={openDetail} />}
        {nav === "apply" && !appDetailId && <Applications applications={applications} setApplications={setApplications} onOpen={setAppDetailId} />}
        {nav === "apply" && appDetailId && <ApplicationDetail app={applications.find(a => a.id === appDetailId)} setApplications={setApplications} experiences={experiences} outputs={outputs} metrics={metrics} onBack={() => setAppDetailId(null)} onOpenExp={openDetail} />}
...
/* ============================================================ 보관함 */
function Archive({ experiences, setExperiences, onOpen, onAnalyze, onGoImport }) {
              ...
              <Card key={e.id} onClick={() => onOpen(e.id)}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <Badge label={STATUS_LABEL[e.status]} color={STATUS_COLOR[e.status][0]} bg={STATUS_COLOR[e.status][1]} />
                    <span onClick={ev => { ev.stopPropagation(); if (window.confirm(`"${e.title}"을(를) 삭제할까요? 되돌릴 수 없습니다.`)) deleteExp(e.id); }}
                      title="삭제" style={{ cursor: "pointer", color: C.faint, fontSize: 13 }}>✕</span>
                  </div>
                </div>
...
/* ============================================================ 경험 상세 */
function ExperienceDetail({ exp, metrics, outputs, setOutputs, setExperiences, onBack, onAnalyze, onDeleted }) {
  ...
  const deleteExp = () => { if (window.confirm(`"${exp.title}"을(를) 삭제할까요? 되돌릴 수 없습니다.`)) { setExperiences(prev => prev.filter(e => e.id !== exp.id)); onDeleted(); } };
    ...
    <div style={{ maxWidth: 820 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <span onClick={deleteExp} title="이 경험 삭제" style={{ cursor: "pointer", color: C.faint, fontSize: 14, padding: "0 4px" }}>✕</span>
        </div>
      </div>
...
function ImportFlow({ setExperiences, setSkills, setCerts, setResumeProfile, onDone, experiences }) {
  ...
  const extract = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.error?.message || `API 오류 (HTTP ${res.status})`);
      }
...
/* ============================================================ 역량·스킬 */
function Skills({ skills, setSkills, certs, setCerts, experiences, onOpenExp }) {
  ...
  const removeSkill = (skillId) => setSkills(p => p.filter(s => s.id !== skillId));
  const addCert = () => {
  ...
  const removeCert = (certId) => setCerts(p => p.filter(c => c.id !== certId));
...
/* ============================================================ 지원 관리 */
function Applications({ applications, setApplications, onOpen }) {
  const addApp = () => {
  ...
  const deleteApp = (id) => setApplications(prev => prev.filter(a => a.id !== id));
    ...
    <div>
      ...
      {applications.map(a => (
        <Card key={a.id} onClick={() => onOpen(a.id)} style={{ marginBottom: 10, display: "grid", gridTemplateColumns: "1.5fr 1fr 100px 130px 130px 20px", alignItems: "center", gap: 10 }}>
          ...
          <span onClick={ev => { ev.stopPropagation(); if (window.confirm(`"${a.company}" 지원 항목을 삭제할까요?`)) deleteApp(a.id); }}
            title="삭제" style={{ cursor: "pointer", color: C.faint, fontSize: 13 }}>✕</span>
        </Card>
      ))}
...
function ApplicationDetail({ app, setApplications, experiences, outputs, metrics, onBack, onOpenExp }) {
  ...
  const deleteApp = () => { if (window.confirm(`"${app.company}" 지원 항목을 삭제할까요?`)) { setApplications(prev => prev.filter(a => a.id !== app.id)); onBack(); } };
  ...
  return (
    <div style={{ maxWidth: 880 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 4 }}>
        ...
        <span onClick={deleteApp} title="이 지원 삭제" style={{ cursor: "pointer", color: C.faint, fontSize: 14, padding: "4px" }}>✕</span>
      </div>
      ...
      {tab === "공고 분석" && (
        <Card>
          ...
          {app.requirements.map(r => {
            ...
            const removeReq = () => setApplications(prev => prev.map(a => a.id === app.id
      ...
      {tab === "자소서" && !chatEssayId && (
        <div style={{ display: "grid", gap: 12 }}>
          {app.essays.map(q => {
            ...
            const removeQ = () => setApplications(prev => prev.map(a => a.id === app.id
      ...
      {tab === "면접" && (
        <div style={{ display: "grid", gap: 12 }}>
          {app.interviews.map(iq => {
            ...
            const removeIq = () => setApplications(prev => prev.map(a => a.id === app.id
