...
`아래는 정형화되지 않은 이력서/경험 정리 문서다. 원문은 자유 텍스트일 수도, 표/CSV 형식(엑셀에서 변환됨)일 수도 있다. 내용을 추출해 JSON으로만 응답하라. 마크다운 백틱 없이 순수 JSON만.
규칙:
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: "user", content:
`아래는 채용공고 원문이다. 이 공고에서 요구하는 역량·경험·자격 요건을 추출해 JSON으로만 응답하라. 마크다운 백틱 없이 순수 JSON만.
규칙:
